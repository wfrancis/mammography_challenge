...
# ─── CLINICAL GUARDRAILS ───────────────────────────────────────────────────────
def _template_key(s: str) -> str:
    """Normalize report text for high-confidence train/test template matching."""
    if pd.isna(s):
        return ""
    s = str(s).strip().lower()
    s = s.replace("\r\n", "\n").replace("\r", "\n")
...
def apply_template_overrides(train_df, test_df, pred_values):
    exact_lookup = _build_majority_lookup(train_df, _exact_key, min_count=1, min_purity=1.0)
    template_lookup = _build_majority_lookup(train_df, _template_key, min_count=3, min_purity=0.98)
    out = np.asarray(pred_values, dtype=int).copy()
    exact_hits = 0
    template_hits = 0
    changed = 0

    for i, report in enumerate(test_df["report"]):
        label = exact_lookup.get(_exact_key(report))
        source = "exact"
        if label is None:
            label = template_lookup.get(_template_key(report))
            source = "template"
        if label is None:
            continue
        if source == "exact":
    ...
    print(
        f"Template overrides: exact_hits={exact_hits}, "
        f"template_hits={template_hits}, changed={changed}"
    )
    return out
...
test["target"] = apply_template_overrides(train, test, test["target"].values)
